...
export default function SearchComponent({ heading , subHeading }) {
  ...
  return (
    ...
    >
      <h2
        className={`text-3xl md:text-3xl font-black  text-center  drop-shadow-sm tracking-wide ${
          subHeading ? "text-black mb-4 x" : "mb-8 "
      ...
      <h2
        className={` md:text-xl md:px-40 text-black text-center mb-8  tracking-wide ${
      ...
      <div className="max-w-7xl mx-auto bg-white shadow-2xl  p-8 transition-all duration-500">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3 animate-fadeUp delay-100">
          ...
          <button
            disabled={loading}
            className={`relative cursor-pointer transition mx-auto justify-center overflow-hidden text-white px-10 py-3 rounded-full font-semibold shadow-md group flex items-center ${
              loading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:scale-105"
            }`}
            onClick={() => {
              ...
              let url = `/search/type-${selectedType.replace(/\s+/g, "-").toLowerCase()}/country-${selectedCountry.replace(/\s+/g, "-").toLowerCase()}`;
              if (product) url += `/product-${product.replace(/\s+/g, "-").toLowerCase()}`;
