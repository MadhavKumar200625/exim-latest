...
if (!/^\d{10,}$/.test(mobile)) {
    ...
    try {
      setLoading(true);
      const res = await axios.post("/api/aboutEmail", {
        name,
        email,
      message,
      mobile,
  ...
  return (
   ...
   <div>
          ...
          <div className="md:col-span-3 flex justify-center mt-4">
            <button  onClick={aboutEmail} className="flex items-center gap-2 px-6 py-3 bg-blue-600 cursor-pointer hover:scale-105 text-white   transition-all duration-200">
              Send Message
            </button>
          </div>
        </form>
