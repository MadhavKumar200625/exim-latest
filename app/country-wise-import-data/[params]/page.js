...
export async function generateMetadata({ params }) {
  const slug = params.params;  // ✅ FIXED
   let country = countriesData[slug +"_import_section"];
  console.log(country);

  if (!country) {
    return {
      title: `${slug.toUpperCase()} Import Data | ${slug.toUpperCase()} Customs Data - Exim Trade Data`,
      description: `Get verified ${slug} Import Data, ${slug} customs data and Cameroon shipment data including importers, exporters, trade patterns, HS codes and port information at Exim Trade Data.`,
      keywords:  [
        `${slug} Import Data`,
        `${slug} Customs Data`,
        `${slug} Shipment Data`,
  ...
  return {
    title: country.meta?.title ||`${slug.toUpperCase()} Import Data | ${slug.toUpperCase()} Customs Data - Exim Trade Data`,
    description: country.meta?.description ||`Get verified ${slug} Import Data, ${slug} customs data and Cameroon shipment data including importers, exporters, trade patterns, HS codes and port information at Exim Trade Data.`,
    ...
    alternates: {
      canonical: country.meta?.canonical || `https://eximtradedata.com/country-wise-${slug}-import-data`,
    },
    openGraph: {
      title: country.openGraph?.title || country.meta?.title,
      type: "website",
      url: country.openGraph?.url ||`https://eximtradedata.com/country-wise-${slug}-import-data`,
    ...
    twitter: {
      ...
      site: "@eximtradedata",
      creator: "@eximtradedata",
      url: country.twitter?.url || `https://eximtradedata.com/country-wise-${slug}-import-data`,
